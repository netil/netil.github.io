...
# 체크 리스트
- 성능? <!-- .element: class="fragment size40" -->
- 특정 단말기에서의 버그/이슈 처리? <!-- .element: class="fragment size40" -->
- 안정성 보장은 어떻게? <!-- .element: class="fragment size40" -->
- 지속적 업데이트(기능개선, 버그픽스, 등) 수행 계획은? <!-- .element: class="fragment size40" -->
...
# 체크 리스트
- 컴포넌트? 모듈? 라이브러리? <!-- .element: class="fragment size40" -->
- 충분한 성능과 환경에서 검증이 되었나? <!-- .element: class="fragment size40" -->
- 꾸준한 업데이트가 이루어지는가? <!-- .element: class="fragment size40" -->
...
https://naver.github.io/egjs/latest/doc/eg.html#isHWAccelerable  <!-- .element: style="font-size:16px" -->

### 2) Transition 속성 사용환경 구분 
<pre><code class="transform" style="margin:0 auto">  // CSS transition 사용이 적절한 환경 여부를 반환
  eg.isTransitional();
</code></pre>
https://naver.github.io/egjs/latest/doc/eg.html#isTransitional  <!-- .element: style="font-size:18px" -->
